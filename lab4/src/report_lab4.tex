...
% NOTE: phần này theo yêu cầu báo cáo được để trống để sinh viên tự điền khi
% thực hành trên kit thật. Có thể chèn ảnh Quartus II (pin assignment,
% compilation report) và ảnh chụp board DE2 đang chạy vào các vị trí bên
% dưới sau khi hoàn thành.

\subsubsection{Tạo project và biên dịch trên Quartus II}

% [Chèn ảnh khởi tạo project Quartus / compile report tại đây]

\vspace{2cm}

\subsubsection{Pin assignment}

% [Chèn ảnh Pin Planner sau khi import DE2_pin_assignments.qsf tại đây]

\vspace{2cm}

\subsubsection{Nạp bit-stream xuống DE2 qua USB-Blaster}

% [Chèn ảnh Programmer / cửa sổ Hardware Setup tại đây]

\vspace{2cm}

\subsubsection{Kiểm thử đọc/ghi trên board}

% [Chèn ảnh chụp board DE2 đang hiển thị HEX7..HEX0 với các cặp (addr, data) đã ghi/đọc thành công tại đây]

\vspace{2cm}

\subsubsection{Nhận xét}

% [Sinh viên ghi nhận xét về: sự khác biệt giữa chip SRAM ngoài so với M4K IP core nội bộ, các vấn đề gặp phải khi xử lý bus inout trên phần cứng thực, chất lượng hiển thị trên 7-seg, v.v.]
...
\begin{enumerate}
    ...
    \item \textbf{External SRAM DE2} (\texttt{IS61LV25616AL-10}): top-level $32 \times 8$ + behavioral model + testbench. Kết quả mô phỏng: \textbf{34/34 PASS}. Phần kiểm thử trên board DE2 thực tế hoàn thành trong buổi thực hành.
\end{enumerate}
